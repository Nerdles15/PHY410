...
int main()
{
  using namespace std; 
    ...
    // data downloaded from http://www.quake.geo.berkeley.edu/anss/catalog-search.html
    const string quake_data("anss_earthquake_catalog.csv");
    ...
    ofstream histogram_file("histogram2.dat");
    for (int i = 0; i < M_values.size(); i++)
        histogram_file << M_values[i] << '\t' << log10N_values[i] << '\n';
    histogram_file.close();
    cout << " wrote events to file: " << "histogram2.dat" << endl;
